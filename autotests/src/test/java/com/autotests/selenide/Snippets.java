package com.autotests.selenide;
...
public class Snippets {
  ...
  void browser_command_examples() {
    ...
    open("/customer/orders");     // -Dselenide.baseUrl=http://123.23.23.1 Configuration.baseUrl=
    open("/", AuthenticationType.BASIC,
            new BasicAuthCredentials("", "user", "password"));

    Selenide.back();    //возврат на страницу назад
    ...
    open("http://testserver/empty.txt");
    var cookie = new Cookie("foo", "bar");
    WebDriverRunner.getWebDriver().manage().addCookie(cookie);
    open("http://testserver/recommendation");
  ...
  void actions_examples() {
    ...
    $("").setValue("text");  //удалится в поле и напишется заново
    $("").append("text");  //дописать в конец
    $("").clear();
    $("").setValue(""); // clear
  ...
  void conditions_examples() {
    ...
    $("").shouldHave(cssClass("red"));  //проверка что есть класс red
    ...
    $("").should(exist);  //проверить есть ли элемент в коде сайта, даже если он невидим для пользователя
  ...
  void collections_examples() {
    ...
    $("div", 1).click(); // same as previous если коллекция
    ...
    $$("").shouldHave(texts("Alfa", "Beta", "Gamma"));   //текст по построению
    $$("").shouldHave(exactTexts("Alfa", "Beta", "Gamma"));  //точный текст
